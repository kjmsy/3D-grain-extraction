...
sym_num = 0 #0 to 23
grain_ni = 1 # grain number start
grain_nf = 2 # grain number end
...
#%%
# -----------------------------
# Cubic symmetry operators (proper rotations, 24)
# -----------------------------
...
xyz_vstack = np.vstack((x_a, y_a, z_a))

cubic_s = cubic_symmetries[sym_num]
print(cubic_s)
xyz_n = np.matmul(cubic_s, xyz_vstack)
x_a, y_a, z_a = xyz_n
x_a = x_a - np.min(x_a)
y_a = y_a - np.min(y_a)
z_a = z_a - np.min(z_a)
